...
def _extract_json(raw):
    # find the first balanced {...} block, tolerate prose/backticks around it
    depth=0; start=-1
    ...
    raise ValueError("no JSON object in: "+raw[:200])
...
def call_ybf(c,txt,temp):
    ...
    m=c.messages.create(model=MODEL,max_tokens=512,temperature=temp,system=CONSTITUTION,
        messages=[{"role":"user","content":f"ACTION:\n{txt}\n\nScore the five axes now. Output ONLY the JSON object."}])
    obj=_extract_json(m.content[0].text)
